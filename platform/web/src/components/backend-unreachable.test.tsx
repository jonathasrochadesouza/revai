...
describe("buildTroubleshootingPrompt", () => {
  it("interpolates the reason and names the port and health route", () => {
    ...
    expect(prompt).toContain("uv run revai-api");
  });
});
...
describe("BackendUnreachable", () => {
  it("never renders the AI troubleshooting prompt into the DOM", async () => {
    ...
    // user-facing recovery commands DO render, as the recovery hint.
    ...
    expect(document.body.textContent).toContain("uv run revai-api");
    ...
    expect(copied).toContain("uv run revai-api");
