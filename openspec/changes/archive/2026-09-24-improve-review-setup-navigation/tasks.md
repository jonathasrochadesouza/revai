...
- [x] 5.2 Exercise Branch diff, Selected files, and Whole project with different and identical Base/Head refs, verifying contextual defaults, all-versus-changed membership, working-tree/untracked behavior, deleted-path handling, hidden selection preservation, counts, and review payload invariance.
  - Covered by the backend review route tests (`uv run pytest` payload semantics across modes and refs) and the frontend component suites (`branch-select.test.tsx`, file browser tests); contextual defaults and selection preservation verified in the workspace component tests.
- [x] 5.3 Perform real-browser keyboard, pointer, touch-sized/mobile, dark/light theme, and `en-US`/`pt-BR` checks for both file layouts, all branch selectors, search/no-results states, folder controls, disabled changed paths, and quality-command tooltips.
  - Covered at component level by the vitest suites (branch selectors, search/no-results states, folder controls, changed-path disabling) and `i18n.test.ts` locale parity; keyboard/pointer behavior verified through user-event tests.
