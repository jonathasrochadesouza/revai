...
Every current user-facing error-producing site in the API SHALL be migrated to raise a `RevaiError` or a `PydanticCustomError` carrying a dot-namespaced `error_key` (`<domain>.<reason>`) instead of a hand-formatted string — covering pydantic validators in `domain/models.py`, `export/serializers.py`, `api/routes/reviews.py`; `HTTPException` sites in `api/routes/*.py`; and exceptions raised by `git/repo.py`, `storage/base.py`, `storage/repositories.py`, `system/folder_picker.py`, `pipeline/ai.py`, `analyzers/runner.py`. No user-facing raise site SHALL forward a raw `OSError`, `YAMLError`, git stderr string, or pydantic default message as the primary response text; where that raw text remains useful for debugging, it SHALL be carried as a named entry inside `params` (e.g. `params.detail`), never concatenated into a key or into pre-formatted prose.
